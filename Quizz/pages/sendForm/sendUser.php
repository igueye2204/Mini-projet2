...
if(isset($_POST['prenom'], $_POST['nom'], $_POST['login'], $_POST['password'], $_FILES['avatar']) && !empty($_POST['prenom']) && !empty($_POST['nom']))
{
        ...
        $firstname       = stripslashes(trim($_POST['prenom']));
        $name            = stripslashes(trim($_POST['nom']));
        $login           = stripslashes(trim($_POST['login']));
        $password        = stripslashes(trim($_POST['password']));
        $passwordconfirm = stripslashes(trim($_POST['passwordconfirm']));
    ...
    $statement = $connection->prepare("INSERT INTO users (firstname,name,login,password,image) VALUES (?,?,?,?,?)");
    $statement->execute(array($firstname,$name,$login,$password,$avatar));
